...
const sessiondevice = () => {
  type: type.SESSIONDEVICE, device
}

const sessiondevicename = () => {
  type: type.SESSIONDEVICENAME, device.name
}

export const pairDevice = device => dispatch => {
...
export const enabledevice = () => dispatch => {
...
export const bluetoothSession = () => dispatch => {
  ...
  BluetoothSerial.on('connectionLost', () => {
    if (dispatch(sessiondevice())) {
      Toast.showShortBottom(
        `Connection to device ${dispatch(sessiondevicename())} has been lost`
      )
    }
    dispatch(connectedFalse())
  })
